...
/*
const unsigned long eventInterval = 100;//1 second or 1,000 ms
const unsigned long IntervalMTRVal = 100;
unsigned long PrevousMTRVal = 0;
...
//float MaxWidth;
int once = 1; //for aligning the robot to wall 
...
{  
  ...
  while( Xval == 0 && Yval == 0){
   ...
   if(once == 1 ) //run once
   {
     ...
     once = 0;  //prevent it running again in the next loop
  ...
  Serial.print("cm\n");
  Serial.print("\nRight ");
  Serial.print(RightSensor.FindDistance());
  Serial.print("cm\n");
  //Serial.print("\nBack ");
  //Serial.print(BackSensor.FindDistance());
//Serial.print("cm\n");
  delay(2000);
  */
